	...
	*/
	...
	$aTransactions = parcelcheckout_database_getRecords($sql);
		...
		{			
			...
			<div onmousedown="disableLowerDragging()" onmouseup="enableLowerDragging()" ontouchstart="disableLowerDragging()" ontouchend="enableLowerDragging()" class="lowerWidgetContent ordersContent">
			...
			{				
				...
				$sHtml .= '
					<tr>
						<td><span id="opener' . $orderId . '" style="text-decoration:none; color:#00acd6;">' . $orderId . '</span></td>
						
						<script>
						$( function() {
							$( "#dialog' . $orderId . '" ).dialog({
							  autoOpen: false,
							...
							$( "#opener' . $orderId . '" ).on( "click", function(){
								$( "#dialog' . $orderId . '" ).dialog( "open" );
							});
						});
						</script>
						
						<div id="dialog' . $orderId . '" title="Order: ' . $orderId . '">
							<p>Order: "' . $orderId . '"<br>
	...
	$sHtml .= '
			...
			<div class="ordersPremiumNotification">
				...
				Upgrade <a href="#">HIER</a> naar premium.
			</div>
	';
