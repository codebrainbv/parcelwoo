...
class WC_Parcelcheckout_Letterbox extends WC_Shipping_Method 
{	
    function __construct($iInstanceId = 0) 
	{
        $this->instance_id        = absint($iInstanceId);
        $this->id                 = 'parcelcheckout_letterbox';
	...
	{		
        if($oMethod->method_id == 'parcelcheckout_letterbox')
		{
            $aChosenMethod = WC()->session->get('parcelcheckout_letterbox');
