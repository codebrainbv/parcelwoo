...
class WC_Parcelcheckout_ShipmentRow extends WC_Shipping_Method 
{	
    function __construct($iInstanceId = 0) 
	{
        $this->instance_id        = absint($iInstanceId);
        $this->id                 = 'parcelcheckout_shipmentrow';
        $this->method_title       = __('PC Letterbox delivery', 'woocommerce-parcelcheckout');
	...
	{		
        if($oMethod->method_id == 'parcelcheckout_shipmentrow')
		{
            $aChosenMethod = WC()->session->get('parcelcheckout_shipmentrow');
