...
class WC_Parcelcheckout_ShipmentEu extends WC_Shipping_Method 
{	
    function __construct($iInstanceId = 0) 
	{
        $this->instance_id        = absint($iInstanceId);
        $this->id                 = 'parcelcheckout_shipmenteu';
	...
	{		
        if($oMethod->method_id == 'parcelcheckout_shipmenteu')
		{
            $aChosenMethod = WC()->session->get('parcelcheckout_shipmenteu');
