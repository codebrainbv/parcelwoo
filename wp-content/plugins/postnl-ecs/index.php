<?php
	...
	*/
	...
	function my_custom_checkout_field_display_admin_order_meta($order) {
		...
		if (strlen($trackcode) == 0) {
			...
			foreach ($codes as $code) {
				$url = $tracking . '/' . $code . '/' . $order->get_billing_country() . '/' . $order->get_billing_postcode();
				if ($tracking == '') {
					$url = 'https://jouw.postnl.nl/#!/track-en-trace/' . $code . '/' . $order->get_billing_country() . '/' . $order->get_billing_postcode();
				}
	...
	function woo_add_order_notes_to_email() {
		...
		$trackcode = get_post_meta($post->ID, 'trackAndTraceCode', true);
		if (strlen($trackcode) == 0) {
			...
			foreach ($codes as $code) {
				$url = 'https://jouw.postnl.nl/#!/track-en-trace/' . $code . '/NL/' . $order->get_billing_postcode();
				echo ' <a target="_blank" href=' . $url . ' >' . $code . '</a>';
			}
		}
	}
	...
	function some_order_meta_keys() {
		if (get_post_meta(get_the_ID(), 'trackAndTraceCode')) {
			$trackcode = get_post_meta($post->ID, 'trackAndTraceCode', true);
			if (strlen($trackcode) == 0) {
			
			} else {
				$codes = explode(";", $trackcode);
				echo '<p><strong>' . __('Track & Trace code') . ':</strong> </p>';
				global $wpdb;
				$table_name_ecs = $wpdb->prefix . 'ecs';
				// find list of states in DB
				$qry = "SELECT * FROM   $table_name_ecs " . "WHERE keytext ='shipmentImport' ORDER BY id DESC  LIMIT 1 ";
				...
				foreach ($codes as $code) {
					$url = $tracking . '/' . $code . '/NL/' . $order->get_billing_postcode();
					if ($tracking == '') {
						$url = 'https://jouw.postnl.nl/#!/track-en-trace/' . $code . '/NL/' . $order->get_billing_postcode();
					}
					echo ' <a target="_blank" href=' . $url . ' >' . $code . '</a>';
	...
	add_action('task_shipement_import', 'cron_shipement_import');
	...
	function cron_shipement_import() {
		orderfunction12();
		$obj = new ni_order_list();
		$obj->shipmentImport();
	}
